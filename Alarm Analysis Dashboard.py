...
# FIX: Wrap xlrd in try/except so the app doesn't crash on startup
# if the user never uploads .xls files and xlrd is not installed.
try:
    import xlrd
    XLRD_AVAILABLE = True
except ImportError:
...
# ----------------------------
# FIX: All helper functions moved to top (before Streamlit UI calls)
# so code structure is: imports → helpers → UI logic
# ----------------------------
...
# FIX: Added @st.cache_data so Excel files are not re-parsed on every
# Streamlit rerun (e.g. when the user adjusts a sidebar filter).
# The cache key is the raw file bytes + filename, so a new upload busts it.
...
@st.cache_data(show_spinner=False)
def load_selected_sheet(file_bytes: bytes, file_name: str, sheet_name: str):
    name = file_name.lower()

    # Try calamine first (fastest); falls through silently if not installed.
    try:
        return pd.read_excel(
            io.BytesIO(file_bytes), sheet_name=sheet_name, engine="calamine"
        )
    except Exception:
        pass  # python-calamine not installed — falling back to standard engines
    ...
    if name.endswith(".xls"):
        ...
        try:
            return pd.read_excel(
                io.BytesIO(file_bytes), sheet_name=sheet_name, engine="xlrd"
            )
        except Exception:
            book = xlrd.open_workbook(
                file_contents=file_bytes,
...
def add_download_button(fig, file_name, label):
    try:
        ...
        # FIX: English message with actionable install instruction
        st.info("PNG export requires kaleido. Install it with: pip install kaleido")
...
def add_title_slide(prs, title, subtitle=""):
    ...
    # FIX: Guard against layouts that have no title placeholder
    try:
...
# FIX: Extracted shared metric computation to eliminate ~80 lines of
# duplication between add_summary_slide and add_template_summary_slide.
def _compute_summary_sections(df):
    """Return (sections, metrics) dict shared by both summary slide builders."""
    ...
    top_level = (
        df["Level"].value_counts().idxmax()
        if "Level" in df.columns and not df["Level"].dropna().empty
        else "N/A"
    )
    top_biu = (
        df["BIU"].value_counts().idxmax()
...
def _write_sections_to_text_frame(tf, sections, heading_pt=20, bullet_pt=16):
    """Write section headings + bullets into a python-pptx text frame."""
    tf.clear()
    first_written = False
...
def add_summary_slide(prs, df, source_file, sheet_name):
    ...
    # FIX: Guard against slide layouts with no title placeholder
    try:
        if slide.shapes.title:
    ...
    sections = _compute_summary_sections(df)
    body = slide.shapes.placeholders[1].text_frame
    _write_sections_to_text_frame(body, sections, heading_pt=20, bullet_pt=16)
...
def add_template_summary_slide(prs, df, source_file, sheet_name):
    ...
    try:
        ...
        _write_sections_to_text_frame(target_tf, sections, heading_pt=19, bullet_pt=16)
...
def build_editable_ppt_report(df, source_file, sheet_name, template_bytes=None):
    ...
    # FIX: Compute Level data once and reuse for both bar and pie slides
    if "Level" in df.columns and not df.empty:
        level_x = df["Level"].value_counts().reset_index()
        level_x.columns = ["Level", "Count"]
        level_x["Count"] = level_x["Count"].astype(int)
        add_native_chart_slide(
            prs, "Alarm Count by Level",
            level_x["Level"], level_x["Count"],
            chart_type=XL_CHART_TYPE.COLUMN_CLUSTERED
        )
        add_native_chart_slide(
            prs, "Alarm Distribution by Level",
            level_x["Level"], level_x["Count"],
    ...
    if "Duration (Minutes)" in df.columns and df["Duration (Minutes)"].notna().any():
        ...
        x["Count"] = x["Count"].astype(int)
        add_native_chart_slide(
            prs, "Alarm Duration Distribution",
            x["Range"], x["Count"],
            chart_type=XL_CHART_TYPE.COLUMN_CLUSTERED
        )
    ...
    if "Level" in df.columns and "BIU" in df.columns and not df.empty:
        ...
        heat_pivot = (
            heat_df.pivot(index="Level", columns="BIU", values="Count")
...
st.markdown(
    "Upload multiple Excel files, choose a sheet, visualize charts, "
    "and download an editable PowerPoint report."
)
...
if uploaded_files:
    ...
    for uf in uploaded_files:
        try:
            sheets = load_workbook_sheets(uf.getvalue(), uf.name)
            workbook_map[uf.name] = {"file": uf, "sheets": sheets}
    ...
    if valid_workbooks:
        ...
        try:
            uf_obj = workbook_map[selected_workbook]["file"]
            raw_df = load_selected_sheet(uf_obj.getvalue(), uf_obj.name, selected_sheet)
            df = clean_dataframe(raw_df)

            st.success(
                f"Loaded workbook: **{selected_workbook}** | Sheet: **{selected_sheet}**"
            )
            ...
            if "Level" in filtered_df.columns:
                ...
                selected_levels = st.sidebar.multiselect(
                    "Select Level", level_options, default=level_options
                )
            ...
            if "BIU" in filtered_df.columns:
                ...
                selected_bius = st.sidebar.multiselect(
                    "Select BIU", biu_options, default=biu_options
                )
            ...
            if "Device" in filtered_df.columns:
                ...
                selected_devices = st.sidebar.multiselect(
                    "Select Device", device_options, default=device_options
                )
            ...
            if "Name" in filtered_df.columns:
                ...
                selected_names = st.sidebar.multiselect(
                    "Select Name", name_options, default=name_options
                )
            ...
            unique_levels = (
                filtered_df["Level"].nunique() if "Level" in filtered_df.columns else 0
            )
            unique_bius = (
                filtered_df["BIU"].nunique() if "BIU" in filtered_df.columns else 0
            )
            avg_duration = (
                round(filtered_df["Duration (Minutes)"].dropna().mean(), 2)
                if "Duration (Minutes)" in filtered_df.columns
                and filtered_df["Duration (Minutes)"].notna().any()
                else 0
            )
            ...
            if "Level" in filtered_df.columns and not filtered_df.empty:
                ...
                fig1 = px.bar(
                    level_count, x="Level", y="Count", text="Count",
                    title="Alarm Count by Level"
                )
            ...
            if "BIU" in filtered_df.columns and not filtered_df.empty:
                ...
                fig2 = px.bar(
                    biu_count, x="BIU", y="Count", text="Count",
                    title="Alarm Count by BIU"
                )
                fig2.update_traces(textposition="outside", texttemplate="%{text:.0f}")
                fig2.update_layout(
                    xaxis_title="BIU", yaxis_title="Count",
                    yaxis=dict(tickformat="d")
                )
            ...
            if "Level" in filtered_df.columns and not filtered_df.empty:
                ...
                add_download_button(
                    fig_pie, "alarm_distribution_by_level_pie.png", "Download Pie Chart"
                )
            ...
            if "Device" in filtered_df.columns and not filtered_df.empty:
                device_count = (
                    filtered_df["Device"].value_counts().reset_index().head(20)
                )
                device_count.columns = ["Device", "Count"]
                device_count["Count"] = device_count["Count"].astype(int)

                fig3 = px.bar(
                    device_count, x="Device", y="Count", text="Count",
                ...
                add_download_button(
                    fig3, "alarm_count_by_device.png", "Download Device Chart"
                )
            ...
            if "Name" in filtered_df.columns and not filtered_df.empty:
                ...
                fig4 = px.bar(
                    name_count, x="Name", y="Count", text="Count",
                    title="Top Alarm Names"
                )
            ...
            # Timeline
            if (
                "Time Received" in filtered_df.columns
                and filtered_df["Time Received"].notna().any()
            ):
                ...
                timeline_count = (
                    timeline_df.groupby("Date").size().reset_index(name="Count")
                )
            ...
            # Duration
            if (
                "Duration (Minutes)" in filtered_df.columns
                and filtered_df["Duration (Minutes)"].notna().any()
            ):
                ...
                fig6.update_layout(
                    xaxis_title="Duration (Minutes)", yaxis_title="Frequency"
                )
                fig6 = apply_theme(fig6)
                st.plotly_chart(fig6, use_container_width=True, config=plot_config)
                add_download_button(
                    fig6, "duration_distribution.png", "Download Duration Chart"
                )

            # Heatmap
            if (
                "Level" in filtered_df.columns
                and "BIU" in filtered_df.columns
                and not filtered_df.empty
            ):
                st.subheader("Heatmap View")
                heat_df = (
                    filtered_df.groupby(["Level", "BIU"]).size().reset_index(name="Count")
                )
                heat_pivot = (
                    heat_df.pivot(index="Level", columns="BIU", values="Count")
                    .fillna(0)
                ...
                fig8 = go.Figure(
                    data=go.Heatmap(
                        ...
                        hovertemplate=(
                            "Level: %{y}<br>BIU: %{x}<br>Count: %{z:d}<extra></extra>"
                        )
                    )
                )
                fig8.update_layout(
            ...
            if len(hierarchy_cols) >= 2 and not filtered_df.empty:
                ...
                st.plotly_chart(
                    fig_sunburst, use_container_width=True, config=plot_config
                )
                add_download_button(
                    fig_sunburst,
            ...
            if "Message" in filtered_df.columns and not filtered_df.empty:
                ...
                message_count = (
                    filtered_df["Message"].value_counts().reset_index().head(10)
                )
            ...
            if not filtered_df.empty:
                try:
                    template_bytes = (
                        ppt_template.getvalue() if ppt_template is not None else None
                    )
                    ...
                    out_name = (
                        f"Alarm_Report_Editable_"
                        f"{selected_workbook.replace('.', '_')}_{selected_sheet}.pptx"
                    )
                    ...
                    # FIX: English-only success message
                    st.success(
                        "Editable PPT report ready. "
                        "Charts are fully editable inside PowerPoint."
                    )
